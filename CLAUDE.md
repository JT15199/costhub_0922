...
- **修复**：①auditStore 稳定键按来源区分——规则发现用 type+objects（规则生成稳定），AI 发现改用 type+归一化标题（去数字/金额/标点/空格，数字微变 99%→98%、objects 漂移都保持原状态）②AI 发现不再随规则轮 dismiss（保留到用户标记已读/忽略/7 天归档），杜绝"隐藏→重生"循环 ③AI 发现每轮上限 4→2 条 + 提示词加"用户已标记已读的视为已处理不要再提" ④Projects 巡检标记已读/忽略后 dispatch costhub-audit-changed + costhub-insights-changed（Dashboard 监听刷新列表与计数、App badge 实时减少）⑤历史脏数据：稳定键切换后下一轮巡检自动把换皮重复条目归档
- **验证**：tsc -b 0 错；183 vitest 全过（+2 稳定键用例）
### v2.3.19 巡检去重 + 云端审核克制 + 本地任务规划（2026-08-18，用户反馈：本地巡查老重复（查到6条就反复那6条），重复就不要报了；云端审核弹窗不要故意出现/不碍事；可给本地大模型规划任务分析成本）
- **① 巡检语义去重**（autoAudit.ts）：AI 层容易换说法重复规则层/历史发现（实测：AI"驱动板模块价格严重偏离市场均值"≈规则"MNT-3201「驱动板」成本高于均值 99%"）。新增 isDuplicateFinding 三重判定——a) 标题归一化（去数字/金额/百分号/标点/空格）相等或互相包含 b) 4-gram 交叉包含（AI"缺失目标成本设定"vs 规则"未设定目标成本"）c) 「实体」引用（已有发现的「驱动板」等模块名出现在新标题 → 重复）；入库前过滤丢弃，不产生新 unread（+6 测试用例）
- **② 陈旧已读自动归档**（auditStore.ts replaceAuditFindings）：已读超 7 天的发现自动 dismissed，列表不再永远挂着旧几条
- **③ 巡检提示静默**（Dashboard.tsx）：自动触发（挂载/空列表）的巡检不再弹「数据无变化/AI 未重复思考」「AI 深度洞察暂不可用」——只在手动点「AI 自主巡检」按钮时提示
- **④ 云端审核克制**（维持现状并确认）：autoInsight/autoThink 云端调用 preview 模式走 CloudConfirmBar 底部横幅非打断式队列（不弹窗不遮操作），会话级去重同物料只问一次；无待洞察关键物料时横幅不出现——符合用户"不故意出现、出现要有用、不碍事"
- **⑤ 本地大模型规划成本分析任务**（autoThink.ts）：buildThinkOverview 增加 目标达成（领域级超目标列表+未设目标计数）/ 关键模块占比 ≥40% / 跨项目同模块价差 ≥20% 线索；任务 prompt 改为"先规划 2-4 个本地分析任务（目标差距/模块结构/跨项目价差/供应商集中度/数字核实），逐个用本地工具执行，本地能回答就不申请云端，只有决策确实需要外部行情才申请 cloud_market_query"
- **验证**：tsc -b 0 错；181 vitest 全过（+6 去重用例）
...
- **职责区分**：关键物料洞察=行情趋势；AI 洞察=后台分析结论；AI 情报中心=所有待处理事项统一处理（报价差异确认/建议处理/巡检已读）；最近成本变动=事实记录
- **验证**：tsc -b 0 错；184 vitest 全过
