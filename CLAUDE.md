...
- 项目管理按方案三改为固定项目/版本导航工作台：左侧项目栏可收缩，主区顶部展示模块目标达成轨道（无模块图标，点击可筛选 BOM），SKU/报价/目标/快照/措施/复盘仍通过页签切换；BOM 只在内部表格容器滚动，并保留导入、导出、批量选择、参照项目、模块比对等原功能，右侧新增可收缩议价工作区。
- BOM 工作区提供“模块分组 / 全量表格”视图：全量表格为连续可滚动网格，支持行号、多选、搜索和双击进入既有编辑弹窗；默认模块分组不变，避免为了 Excel 体验破坏模块参照和比对流程。
