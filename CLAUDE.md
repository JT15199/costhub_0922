...
- 视觉边界：玻璃只用于应用外壳、导航、卡片、弹层和 AI 侧栏；Ant Design 表格与成本数字保持高不透明度，避免影响日常比价可读性。
- 个性化设置新增“背景氛围”：支持本地 PNG/JPG/WebP（≤2MB）即时预览、更换和恢复默认；图片只写入本机 `localStorage`，禁止网络加载/上传。
- 个性化设置新增“玻璃质感”透明度滑杆（46%–86%）：通过 `costhub_glass_opacity` 本地保存，统一控制应用外壳、卡片与 AI 协作窗的透光层；成本表格维持高不透明度以保证数字可读性。
