import { SupplierNameInput } from './SupplierResourcePool';
...
import { AimOutlined, CheckCircleOutlined, ClockCircleOutlined, DownloadOutlined, FileExcelOutlined, HistoryOutlined, InboxOutlined, PlusOutlined, ReloadOutlined, UploadOutlined, WarningOutlined } from '@ant-design/icons';
import * as XLSX from 'xlsx';
import { createTenderRound, freezeProjectBOMVersion, getNegotiationItems, getTenderDecision, getTenderMatrix, getTenderOverview, importTenderQuoteBatch, saveNegotiationItems, saveTenderDecision, updateNegotiationItemStatus, updateTenderLineMatch, type MatchRelation, type NegotiationItem, type TenderDecision, type TenderMatrixRow, type TenderOffer, type TenderOverview } from '../db';
import { parseTenderQuoteFile, type ParsedTenderQuote } from '../tenderImport';
import { downloadTenderTemplate } from '../excelTemplates';
...
export default function TenderWorkspace({ projectId, project }: TenderWorkspaceProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRoundId(nextOverview.currentRound?.id || 0);
      if (nextOverview.currentRound) setStage(nextOverview.currentRound.stage || '摸底报价');
  ...
  const confirmImport = async () => {
    ...
    setImporting(true);
    try {
      const result = await importTenderQuoteBatch({ project_id: projectId, supplier_name: supplierName.trim(), source_file_name: preview.sourceFileName, source_file_hash: preview.sourceFileHash, round_id: roundId || undefined, stage, total_amount: preview.totalAmount, currency: 'CNY', tax_mode: 'exclusive', pricing_mode: 'one_time', lines: preview.lines });
  ...
  const openNewRound = async () => {
    try { const next = await createTenderRound(projectId, stage); setRoundId(next.id); message.success(`已创建第${next.roundNo}轮 · ${next.name}`); await load(); }
    catch (error: any) { message.error(`创建报价轮次失败：${String(error?.message || error).slice(0, 120)}`); }
  };
  const freezeBOM = async (sourceType: 'award_openbom' | 'final_bom', label: string) => {
    try { const id = await freezeProjectBOMVersion(projectId, { sourceType, stage: label }); message.success(`${label}已冻结为 BOM 版本 #${id}`); }
    catch (error: any) { message.error(`${label}冻结失败：${String(error?.message || error).slice(0, 120)}`); }
  };
  ...
  return <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
    <Card size="small" styles={{ body: { padding: '14px 16px' } }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', alignItems: 'flex-start' }}>
        ...
        <Space wrap>
          ...
          <Button icon={<DownloadOutlined />} onClick={downloadTenderTemplate}>报价模板</Button>
          ...
          <Button icon={<PlusOutlined />} onClick={() => void openNewRound()}>新建报价轮次</Button>
          <Button onClick={() => void freezeBOM('award_openbom', 'Open BOM')}>冻结 Open BOM</Button>
          <Button onClick={() => void freezeBOM('final_bom', '最终 BOM')}>冻结最终 BOM</Button>
    ...
    <Modal title="导入报价预览" open={!!preview} onCancel={() => !importing && setPreview(null)} onOk={() => void confirmImport()} okText="确认导入" confirmLoading={importing} width={820} destroyOnHidden>
      {preview && <div><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 12 }}><div><div style={{ color: 'var(--color-text-secondary)', fontSize: 12 }}>文件</div><div style={{ fontWeight: 600 }}>{preview.sourceFileName}</div></div><div><div style={{ color: 'var(--color-text-secondary)', fontSize: 12 }}>供应商</div><SupplierNameInput size="small" value={supplierName} onChange={setSupplierName} /></div><div><div style={{ color: 'var(--color-text-secondary)', fontSize: 12 }}>所属阶段</div><Radio.Group size="small" value={stage} onChange={event => { setStage(event.target.value); setRoundId(0); }} options={['摸底报价', '谈价', '定点'].map(value => ({ label: value, value }))} /></div></div><Alert type="info" showIcon message={`识别到 ${preview.lines.length} 行，合计 ${money(preview.totalAmount)}；表头第 ${preview.headerRow + 1} 行`} description={preview.warnings.length ? preview.warnings.join('；') : '将按人民币、未税、一口价保存；原始单元格会保留在批次中。'} style={{ marginBottom: 12 }} /><Table size="small" rowKey="source_row" dataSource={preview.lines.slice(0, 8)} pagination={false} columns={[{ title: '来源行', dataIndex: 'source_row', width: 65 }, { title: '模块', dataIndex: 'module_name' }, { title: '器件名称', dataIndex: 'raw_name' }, { title: '规格', dataIndex: 'raw_specs', ellipsis: true }, { title: '数量', dataIndex: 'quantity', width: 60 }, { title: '单价', dataIndex: 'unit_price', width: 90, render: money }, { title: '小计', dataIndex: 'line_total', width: 90, render: money }]} /></div>}
    </Modal>

    <Drawer title="报价行详情" open={!!selectedRow} onClose={() => setSelectedRow(null)} width={560}>
      {selectedRow && <div><div style={{ fontSize: 17, fontWeight: 700 }}>{selectedRow.materialName}</div><div style={{ color: 'var(--color-text-secondary)', margin: '4px 0 14px' }}>{selectedRow.model || '型号待补'} · {selectedRow.specs || '规格待补'}</div><Alert type="warning" showIcon icon={<WarningOutlined />} message="可比判断需基于名称、型号、规格、模块核实" description="同一供应商同键的多条原始报价会聚合展示，但原始行仍可展开追溯；确认关系只更新选中的原始行。" style={{ marginBottom: 14 }} /><Table size="small" rowKey="quoteLineId" pagination={false} dataSource={Object.values(selectedRow.offers)} expandable={{ expandedRowRender: (offer: TenderOffer) => offer.sourceOffers && offer.sourceOffers.length > 1 ? <Table size="small" rowKey="quoteLineId" pagination={false} dataSource={offer.sourceOffers} columns={[{ title: '原始报价行', dataIndex: 'quoteLineId' }, { title: '数量', dataIndex: 'quantity' }, { title: '小计', dataIndex: 'lineTotal', render: money }, { title: '文件', dataIndex: 'sourceFileName', ellipsis: true }]} /> : null }} columns={[{ title: '供应商', dataIndex: 'supplierName' }, { title: '报价', dataIndex: 'lineTotal', render: (value: number, offer: TenderOffer) => <span>{money(value)}{offer.duplicateCount && offer.duplicateCount > 1 ? `（${offer.duplicateCount}行合计）` : ''}</span> }, { title: '关系', dataIndex: 'relationType', render: (value, offer: TenderOffer) => <Select size="small" value={value} loading={matchSaving === offer.quoteLineId} style={{ width: 92 }} options={Object.entries(relationLabel).map(([key, label]) => ({ value: key, label }))} onChange={next => void confirmRelation(offer.quoteLineId, next as MatchRelation)} aria-label={`${offer.supplierName}匹配关系`} /> }, { title: '来源', dataIndex: 'sourceFileName', ellipsis: true }]} /><Divider /><Progress percent={selectedRow.comparableLow ? 100 : 0} size="small" status={selectedRow.comparableLow ? 'success' : 'exception'} format={() => selectedRow.comparableLow ? `最低：${selectedRow.comparableLow.supplierName}` : '待确认可比关系'} /></div>}
    </Drawer>
