import { useEffect, useState } from 'react';
import { Table, Button, Select, Space, Modal, Form, Input, InputNumber, Tag, message, Popconfirm, Tooltip, Checkbox, Card } from 'antd';
import { PlusOutlined, CopyOutlined, DeleteOutlined, EditOutlined, EyeOutlined, AppstoreOutlined, CheckSquareOutlined, CloseSquareOutlined, BarChartOutlined } from '@ant-design/icons';
import ReactECharts from 'echarts-for-react';
import { getProjects, getModules, getModuleItems, saveModule, deleteModule, saveModuleItem, deleteModuleItem, addBOMItem, getParts, getMainCategories, syncModuleItemToBOM, deleteBOMByModuleName, getModuleNameById, deleteBOMItemsByPartIds } from '../db';
import { MAIN_CATEGORIES, SUB_CATEGORIES, getCategoryColor } from '../constants';

export default function ModuleLibrary() {
  const [allMods, setAllMods] = useState<any[]>([]);
  const [mainCats, setMainCats] = useState(MAIN_CATEGORIES);
  useEffect(() => { (async () => { try { setMainCats(await getMainCategories()); } catch(e) {} })(); }, []);
  const [modGroups, setModGroups] = useState<any[]>([]);    // grouped by name: {name, projects: [{project, module, cost, count}]}
  const [activeModName, setActiveModName] = useState<string | null>(null);
  const [activeGroup, setActiveGroup] = useState<any>(null);
  const [expandedPid, setExpandedPid] = useState<number | null>(null);
  const [expandedItems, setExpandedItems] = useState<any[]>([]);
  const [expandedModId, setExpandedModId] = useState<number | null>(null);

  const [modModal, setModModal] = useState(false);
  const [editMod, setEditMod] = useState<any>(null);
  const [selProjectForNewMod, setSelProjectForNewMod] = useState<number | null>(null);
  const [itemModal, setItemModal] = useState(false);
  const [editItem, setEditItem] = useState<any>(null);
  const [targetPid, setTargetPid] = useState<number | null>(null);
  const [projects, setProjects] = useState<any[]>([]);
  const [form] = Form.useForm();
  const [itemSelKeys, setItemSelKeys] = useState<React.Key[]>([]); // 模块器件选择状态
  const [filterProjectId, setFilterProjectId] = useState<number | null>(null); // 项目筛选
  const [modSelIds, setModSelIds] = useState<number[]>([]); // 模块多选状态
  const [compareExpanded, setCompareExpanded] = useState(false); // 详细对比是否展开
  const [compareSelectedProjects, setCompareSelectedProjects] = useState<number[]>([]); // 选中的对比项目
  const [compareData, setCompareData] = useState<any[]>([]); // 对比数据

  useEffect(() => { loadAll(); }, []);
  useEffect(() => { updateModGroups(); }, [filterProjectId, allMods]); // 筛选变化时更新分组

  const loadAll = async () => {
    const projs = await getProjects(); setProjects(projs);
    const all: any[] = [];
    for (const p of projs) {
      const mods = await getModules(p.id);
      for (const m of mods) {
        const items = await getModuleItems(m.id);
        const total = items.reduce((s: number, i: any) => s + (i.cost || 0) * (i.quantity ?? 1), 0);
        all.push({ ...m, project_id: p.id, project_code: p.code, project_name: p.name, project_type: p.project_type, itemCount: items.length, totalCost: total });
      }
    }
    setAllMods(all);
  };

  const updateModGroups = () => {
    // Group by name (根据筛选项目过滤)
    const groups: Record<string, any> = {};
    const filteredMods = filterProjectId ? allMods.filter(m => m.project_id === filterProjectId) : allMods;
    for (const m of filteredMods) {
      if (!groups[m.name]) groups[m.name] = { name: m.name, projects: [] };
      groups[m.name].projects.push({ project_id: m.project_id, project_code: m.project_code, project_name: m.project_name, project_type: m.project_type, module_id: m.id, cost: m.totalCost, count: m.itemCount, description: m.description });
    }
    setModGroups(Object.values(groups));
    // 清空当前选中的模块
    setActiveModName(null); setActiveGroup(null);
  };

  const selectModGroup = async (group: any) => {
    setActiveModName(group.name); setActiveGroup(group);
    setExpandedPid(null); setExpandedItems([]);
  };

  const expandProject = async (pid: number, modId: number) => {
    setExpandedPid(pid); setExpandedModId(modId);
    setExpandedItems(await getModuleItems(modId));
  };

  const handleSaveMod = async () => {
    const v = await form.validateFields();
    await saveModule({ ...editMod, project_id: selProjectForNewMod, ...v });
    setModModal(false); setEditMod(null); loadAll(); message.success('已保存');
  };

  const handleSaveItem = async () => {
    const v = await form.validateFields();
    await saveModuleItem({ ...editItem, module_id: expandedModId, ...v });
    // 同步更新BOM中的器件数量
    if (expandedModId && editItem?.part_id) {
      const moduleName = await getModuleNameById(expandedModId);
      if (moduleName) {
        await syncModuleItemToBOM(expandedModId, editItem.part_id, v.quantity, moduleName);
      }
    }
    setItemModal(false); setEditItem(null);
    if (expandedModId) setExpandedItems(await getModuleItems(expandedModId));
    loadAll(); message.success('已保存并同步到BOM');
  };

  const copyModuleToProject = async () => {
    if (!expandedModId || !targetPid) { message.warning('请先选择目标项目'); return; }
    const srcMod = allMods.find(m => m.id === expandedModId); if (!srcMod) return;
    const newModId = await saveModule({ project_id: targetPid, name: srcMod.name, description: srcMod.description });
    const items = await getModuleItems(expandedModId);
    for (const item of items) {
      let partId = item.part_id;
      if (!partId) {
        const allParts = await getParts(item.part_name, '', '');
        const match = allParts.find((p: any) => p.model === item.part_model);
        partId = match?.id || await (await import('../db')).savePart({ main_category: item.main_category, sub_category: item.sub_category, category: item.main_category, name: item.part_name, model: item.part_model, cost: item.cost, specs: '', projects: '', remark: '' });
      }
      await saveModuleItem({ module_id: newModId, part_id: partId, part_name: item.part_name, part_model: item.part_model, main_category: item.main_category, sub_category: item.sub_category, cost: item.cost, quantity: item.quantity, remark: item.remark });
      if (partId) { try { await addBOMItem(targetPid, partId, item.quantity, srcMod.name, ''); } catch (e) {} }
    }
    message.success(`已复制到目标项目`); setTargetPid(null); loadAll();
  };

  const expTotal = expandedItems.reduce((s: number, i: any) => s + (i.cost || 0) * (i.quantity ?? 1), 0);
  const bySubCat: Record<string, number> = {};
  expandedItems.forEach(i => { const k = i.sub_category || i.main_category || '其他'; bySubCat[k] = (bySubCat[k] || 0) + (i.cost || 0) * (i.quantity ?? 1); });
  const barData = Object.entries(bySubCat).map(([k, v]) => ({ name: k, value: Math.round(v * 100) / 100 })).sort((a, b) => b.value - a.value);

  const barOption = {
    tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' }, formatter: (p: any) => `${p[0].name}<br/>¥${p[0].value.toFixed(2)}` },
    grid: { left: 110, right: 60, top: 5, bottom: 5 },
    xAxis: { type: 'value', name: '¥' },
    yAxis: { type: 'category', data: barData.map(d => d.name), axisLabel: { fontSize: 11 }, inverse: true },
    series: [{ type: 'bar', barWidth: '55%', data: barData.map(d => ({ value: d.value, itemStyle: { color: getCategoryColor(expandedItems.find(i => i.sub_category === d.name)?.main_category || '其他'), borderRadius: [0, 6, 6, 0] } })), label: { show: true, position: 'right', formatter: (p: any) => `¥${p.value.toFixed(2)}`, fontSize: 10 } }],
  };

  const itemCols = [
    { title: '大类', dataIndex: 'main_category', width: 70, render: (v: string) => <Tag color={getCategoryColor(v)} style={{ margin: 0 }}>{v}</Tag> },
    { title: '子类', dataIndex: 'sub_category', width: 90, ellipsis: true },
    { title: '名称', dataIndex: 'part_name', ellipsis: true },
    { title: '型号', dataIndex: 'part_model', width: 140, ellipsis: true },
    { title: '单价', dataIndex: 'cost', width: 80, align: 'right' as const, render: (v: number) => v?.toFixed(2) },
    { title: '数量', dataIndex: 'quantity', width: 55, align: 'center' as const },
    { title: '小计', key: 'sub', width: 85, align: 'right' as const, render: (_: any, r: any) => <b>{(r.cost * r.quantity).toFixed(2)}</b> },
    { title: '备注', dataIndex: 'remark', width: 100, ellipsis: true },
    {
      title: '', width: 55,
      render: (_: any, r: any) => (
        <Space size={0}>
          <Tooltip title="编辑"><Button type="link" size="small" icon={<EditOutlined />} onClick={(e) => { e.stopPropagation(); setEditItem(r); form.setFieldsValue(r); setItemModal(true); }} /></Tooltip>
          <Popconfirm title="删除？将同步删除BOM中的对应器件" onConfirm={async () => {
            await deleteModuleItem(r.id);
            // 同步删除BOM中的器件
            if (expandedModId && r.part_id) {
              const moduleName = await getModuleNameById(expandedModId);
              const projectId = await (await import('../db')).getModuleProjectId(expandedModId);
              if (projectId && moduleName) {
                await (await import('../db')).deleteBOMItemsByPartIds(projectId, [r.part_id], moduleName);
              }
            }
            if (expandedModId) setExpandedItems(await getModuleItems(expandedModId));
            loadAll(); message.success('已删除并同步');
          }}><Button type="link" size="small" danger icon={<DeleteOutlined />} /></Popconfirm>
        </Space>
      ),
    },
  ];

  return (
    <div>
      <div className="page-title"><span className="emoji">📦</span> 模块库</div>

      {/* Module cards - all modules grouped by name */}
      <div className="content-card" style={{ marginBottom: 14, padding: '14px 18px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <Space>
            <AppstoreOutlined style={{ color: '#CF0A2C', fontSize: 16 }} />
            <b style={{ fontSize: 14 }}>全部模块（跨项目）</b>
            {/* 项目筛选 */}
            <Select
              placeholder="筛选项目..."
              allowClear
              style={{ width: 200 }}
              value={filterProjectId || undefined}
              onChange={(v) => setFilterProjectId(v || null)}
              options={projects.map(p => ({ label: `[${p.code}] ${p.name}`, value: p.id }))}
            />
            {filterProjectId && <Tag color="blue" closable onClose={() => setFilterProjectId(null)}>{projects.find(p => p.id === filterProjectId)?.name}</Tag>}
            {/* 一键全选（只在筛选项目后显示） */}
            {filterProjectId && modGroups.length > 0 && (
              <>
                {modSelIds.length === modGroups.filter(g => g.projects.length === 1).length ? (
                  <Button size="small" icon={<CloseSquareOutlined />} onClick={() => setModSelIds([])}>取消全选</Button>
                ) : (
                  <Button size="small" type="primary" ghost icon={<CheckSquareOutlined />} onClick={() => {
                    // 选中所有模块
                    const allIds = modGroups.filter(g => g.projects.length === 1).map(g => g.projects[0].module_id);
                    setModSelIds(allIds);
                  }}>全选</Button>
                )}
              </>
            )}
            {/* 多选和批量删除 */}
            {modSelIds.length > 0 && (
              <>
                <Tag color="orange">{modSelIds.length} 个模块选中</Tag>
                <Popconfirm title={`删除选中 ${modSelIds.length} 个模块？将同步删除BOM中的对应器件`} onConfirm={async () => {
                  for (const modId of modSelIds) {
                    const moduleName = await getModuleNameById(modId);
                    const projectId = await (await import('../db')).getModuleProjectId(modId);
                    await deleteModule(modId);
                    if (projectId && moduleName) {
                      await deleteBOMByModuleName(projectId, moduleName);
                    }
                  }
                  setModSelIds([]);
                  loadAll();
                  message.success('已批量删除');
                }}>
                  <Button size="small" danger icon={<DeleteOutlined />}>批量删除</Button>
                </Popconfirm>
                <Button size="small" onClick={() => setModSelIds([])}>取消选择</Button>
              </>
            )}
          </Space>
          <Button type="primary" size="small" icon={<PlusOutlined />} onClick={() => {
            setEditMod(null); setSelProjectForNewMod(filterProjectId || null); form.resetFields(); setModModal(true);
          }}>新建模块</Button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(210px, 1fr))', gap: 10 }}>
          {modGroups.map(group => {
            const costs = group.projects.map((p: any) => p.cost);
            const minC = Math.min(...costs), maxC = Math.max(...costs);
            const active = activeModName === group.name;
            // 如果有项目筛选，显示具体模块（可以选择）
            if (filterProjectId && group.projects.length === 1) {
              const proj = group.projects[0];
              const isSelected = modSelIds.includes(proj.module_id);
              return (
                <div key={group.name}
                  onClick={() => selectModGroup(group)}
                  style={{
                    padding: '14px 16px', borderRadius: 10, cursor: 'pointer',
                    border: isSelected ? '2px solid #F97316' : active ? '2px solid #CF0A2C' : '1px solid #E8ECF1',
                    background: isSelected ? '#FFF7ED' : active ? 'linear-gradient(135deg, #FFF1F0 0%, #FFF5F5 100%)' : '#FFF',
                    boxShadow: isSelected ? '0 4px 16px rgba(249,115,22,0.12)' : active ? '0 4px 16px rgba(207,10,44,0.12)' : '0 1px 3px rgba(0,0,0,0.04)',
                    transition: 'all 0.2s',
                    position: 'relative',
                  }}
                  onMouseEnter={e => { if (!active && !isSelected) { e.currentTarget.style.borderColor = '#CF0A2C'; e.currentTarget.style.transform = 'translateY(-2px)'; } }}
                  onMouseLeave={e => { if (!active && !isSelected) { e.currentTarget.style.borderColor = '#E8ECF1'; e.currentTarget.style.transform = 'none'; } }}
                >
                  {/* 选择Checkbox */}
                  <Checkbox
                    checked={isSelected}
                    onChange={(e) => {
                      e.stopPropagation();
                      if (e.target.checked) {
                        setModSelIds([...modSelIds, proj.module_id]);
                      } else {
                        setModSelIds(modSelIds.filter(id => id !== proj.module_id));
                      }
                    }}
                    style={{ position: 'absolute', top: 8, right: 8 }}
                  />
                  <div style={{ fontWeight: 600, fontSize: 14, color: isSelected ? '#F97316' : active ? '#CF0A2C' : '#1E293B', marginBottom: 6 }}>{group.name}</div>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                    <Tag style={{ margin: 0, fontSize: 11 }}>{proj.count || 0} 件</Tag>
                    <span style={{ fontSize: 13, color: '#CF0A2C', fontWeight: 600 }}>¥{proj.cost.toFixed(0)}</span>
                  </div>
                </div>
              );
            }
            // 没有筛选项目，显示跨项目分组（不可选择）
            return (
              <div key={group.name}
                onClick={() => selectModGroup(group)}
                style={{
                  padding: '14px 16px', borderRadius: 10, cursor: 'pointer',
                  border: active ? '2px solid #CF0A2C' : '1px solid #E8ECF1',
                  background: active ? 'linear-gradient(135deg, #FFF1F0 0%, #FFF5F5 100%)' : '#FFF',
                  boxShadow: active ? '0 4px 16px rgba(207,10,44,0.12)' : '0 1px 3px rgba(0,0,0,0.04)',
                  transition: 'all 0.2s',
                }}
                onMouseEnter={e => { if (!active) { e.currentTarget.style.borderColor = '#CF0A2C'; e.currentTarget.style.transform = 'translateY(-2px)'; } }}
                onMouseLeave={e => { if (!active) { e.currentTarget.style.borderColor = '#E8ECF1'; e.currentTarget.style.transform = 'none'; } }}
              >
                <div style={{ fontWeight: 600, fontSize: 14, color: active ? '#CF0A2C' : '#1E293B', marginBottom: 6 }}>{group.name}</div>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                  <Tag style={{ margin: 0, fontSize: 11 }}>{group.projects.length} 个项目</Tag>
                  <span style={{ fontSize: 13, color: '#CF0A2C', fontWeight: 600 }}>
                    ¥{minC === maxC ? minC.toFixed(0) : `${minC.toFixed(0)}~${maxC.toFixed(0)}`}
                  </span>
                </div>
              </div>
            );
          })}
        </div>
      </div>

      {/* Module comparison view */}
      {activeGroup && (
        <div className="content-card" style={{ padding: '16px 20px', marginBottom: 14 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            <h3 style={{ margin: 0, fontSize: 16, color: '#CF0A2C' }}>{activeGroup.name} — 各项目成本对比</h3>
            {activeGroup.projects.length > 1 && (
              <Button type="primary" icon={<BarChartOutlined />}
                onClick={async () => {
                  if (!compareExpanded) {
                    // 首次展开：加载所有项目数据，默认选中前两个项目
                    const allItems: any[] = [];
                    for (const proj of activeGroup.projects) {
                      const items = await getModuleItems(proj.module_id);
                      for (const item of items) {
                        allItems.push({
                          ...item,
                          project_id: proj.project_id,
                          project_code: proj.project_code,
                          project_name: proj.project_name,
                          module_cost: proj.cost,
                        });
                      }
                    }
                    setCompareData(allItems);
                    setCompareSelectedProjects(activeGroup.projects.slice(0, 2).map((p: any) => p.project_id));
                    setCompareExpanded(true);
                  } else {
                    setCompareExpanded(false);
                  }
                }}>
                {compareExpanded ? '收起对比' : '详细对比'}
              </Button>
            )}
          </div>
          <Table
            dataSource={activeGroup.projects}
            rowKey="module_id"
            size="small"
            pagination={false}
            columns={[
              { title: '项目代号', dataIndex: 'project_code', width: 110, render: (v: string) => <b>{v}</b> },
              { title: '项目名称', dataIndex: 'project_name', ellipsis: true },
              { title: '类型', dataIndex: 'project_type', width: 75, render: (v: string) => <Tag color={v === '已完成' ? 'green' : 'blue'}>{v}</Tag> },
              { title: '模块成本', dataIndex: 'cost', width: 120, align: 'right' as const, render: (v: number) => {
                const minCost = Math.min(...activeGroup.projects.map((p: any) => p.cost));
                const diff = v - minCost;
                return (
                  <Space direction="vertical" size={0}>
                    <b style={{ color: '#CF0A2C', fontSize: 15 }}>¥{v.toFixed(2)}</b>
                    {diff > 0.01 && <span style={{ fontSize: 11, color: '#EF4444' }}>+¥{diff.toFixed(2)}</span>}
                  </Space>
                );
              }},
              { title: '器件数', dataIndex: 'count', width: 70, align: 'center' as const },
              {
                title: '操作', width: 100,
                render: (_: any, r: any) => (
                  <Button type="link" size="small" icon={<EyeOutlined />}
                    onClick={() => expandProject(r.project_id, r.module_id)}
                    style={{ color: expandedPid === r.project_id ? '#CF0A2C' : undefined }}>
                    查看明细
                  </Button>
                ),
              },
            ]}
          />

          {/* Expanded detail */}
          {expandedPid && expandedItems.length > 0 && (
            <div style={{ marginTop: 16, border: '1px solid #E8ECF1', borderRadius: 8, padding: 14, background: '#FAFBFC' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
                <Space>
                  <b>{activeGroup.name} 明细</b>
                  <Tag color="blue">{expandedItems.length} 件</Tag>
                  <Tag color="red">¥{expTotal.toFixed(2)}</Tag>
                  <span style={{ fontSize: 11, color: '#999' }}>
                    {projects.find(p => p.id === expandedPid)?.code} {projects.find(p => p.id === expandedPid)?.name}
                  </span>
                  {/* 全选/取消全选按钮 */}
                  {(() => {
                    const allSelected = expandedItems.every(i => itemSelKeys.includes(i.id));
                    return allSelected ? (
                      <Button size="small" icon={<CloseSquareOutlined />} onClick={() => setItemSelKeys([])}>取消全选</Button>
                    ) : (
                      <Button size="small" type="primary" ghost icon={<CheckSquareOutlined />} onClick={() => setItemSelKeys(expandedItems.map(i => i.id))}>全选</Button>
                    );
                  })()}
                  {/* 批量删除 */}
                  {itemSelKeys.length > 0 && (
                    <Popconfirm title={`删除选中 ${itemSelKeys.length} 项？将同步删除BOM中的对应器件`} onConfirm={async () => {
                      const selectedItems = expandedItems.filter(i => itemSelKeys.includes(i.id));
                      for (const item of selectedItems) {
                        await deleteModuleItem(item.id);
                        // 同步删除BOM中的器件
                        if (expandedModId && item.part_id) {
                          const moduleName = await getModuleNameById(expandedModId);
                          const projectId = await (await import('../db')).getModuleProjectId(expandedModId);
                          if (projectId && moduleName) {
                            await deleteBOMItemsByPartIds(projectId, [item.part_id], moduleName);
                          }
                        }
                      }
                      setItemSelKeys([]);
                      setExpandedItems(await getModuleItems(expandedModId!));
                      loadAll();
                      message.success('已删除并同步到BOM');
                    }}>
                      <Button size="small" danger icon={<DeleteOutlined />}>批量删除 ({itemSelKeys.length})</Button>
                    </Popconfirm>
                  )}
                </Space>
                <Space>
                  <Select placeholder="复制到项目..." value={targetPid || undefined} onChange={v => setTargetPid(v)} style={{ width: 200 }} size="small"
                    options={projects.filter(p => p.id !== expandedPid).map(p => ({ label: `[${p.code}] ${p.name}`, value: p.id }))} />
                  <Button size="small" icon={<CopyOutlined />} onClick={copyModuleToProject}>复制</Button>
                  <Button type="primary" size="small" icon={<PlusOutlined />} onClick={() => { setEditItem(null); form.resetFields(); form.setFieldsValue({ main_category: '硬件类', sub_category: '', quantity: 1, cost: 0 }); setItemModal(true); }}>添加器件</Button>
                  <Popconfirm title="删除模块？将同步删除BOM中该模块的所有器件" onConfirm={async () => {
                    const moduleName = await getModuleNameById(expandedModId!);
                    const projectId = await (await import('../db')).getModuleProjectId(expandedModId!);
                    await deleteModule(expandedModId!);
                    // 同步删除BOM中该模块的所有器件
                    if (projectId && moduleName) {
                      await deleteBOMByModuleName(projectId, moduleName);
                    }
                    setExpandedPid(null); setExpandedItems([]);
                    loadAll(); message.success('模块已删除，BOM已同步');
                  }}>
                    <Button size="small" danger icon={<DeleteOutlined />}>删除模块</Button>
                  </Popconfirm>
                </Space>
              </div>
              <Table dataSource={expandedItems} columns={itemCols} rowKey="id" size="small" pagination={false} scroll={{ x: 850 }}
                rowSelection={{ selectedRowKeys: itemSelKeys, onChange: (keys) => setItemSelKeys(keys) }}
                summary={() => (<Table.Summary.Row><Table.Summary.Cell index={0} colSpan={6}><b>合计</b></Table.Summary.Cell><Table.Summary.Cell index={6} align="right"><b style={{ color: '#CF0A2C', fontSize: 14 }}>¥{expTotal.toFixed(2)}</b></Table.Summary.Cell><Table.Summary.Cell index={7} colSpan={2} /></Table.Summary.Row>)}
              />
              {barData.length > 0 && (
                <div style={{ maxWidth: 600, margin: '16px auto 0' }}>
                  <div style={{ textAlign: 'center', marginBottom: 6 }}><b style={{ fontSize: 13 }}>📊 子类成本分布</b></div>
                  <ReactECharts option={barOption} style={{ height: Math.max(160, barData.length * 34 + 30) }} />
                </div>
              )}
            </div>
          )}

          {/* Detailed comparison view (expanded below) */}
          {compareExpanded && compareData.length > 0 && activeGroup && (
            <div style={{ marginTop: 16, border: '1px solid #E8ECF1', borderRadius: 8, padding: 14, background: '#FAFBFC' }}>
              <div style={{ marginBottom: 12 }}>
                <b style={{ fontSize: 14 }}>📊 器件详细对比</b>
                <span style={{ marginLeft: 12, fontSize: 12, color: '#999' }}>选择要对比的项目：</span>
                <Checkbox.Group
                  value={compareSelectedProjects}
                  onChange={(vals) => setCompareSelectedProjects(vals as number[])}
                  options={activeGroup.projects.map((proj: any) => ({ label: `${proj.project_code}`, value: proj.project_id }))}
                  style={{ marginLeft: 8 }}
                />
              </div>

              {/* 对比表格 */}
              {compareSelectedProjects.length >= 2 && (() => {
                // 筛选选中项目的数据
                const filteredData = compareData.filter(d => compareSelectedProjects.includes(d.project_id));
                // 按器件名+型号分组
                const grouped: Record<string, any> = {};
                for (const item of filteredData) {
                  const key = `${item.part_name}|${item.part_model || ''}`;
                  if (!grouped[key]) {
                    grouped[key] = { part_name: item.part_name, part_model: item.part_model, main_category: item.main_category, sub_category: item.sub_category, projects: {} };
                  }
                  grouped[key].projects[item.project_id] = { cost: item.cost, quantity: item.quantity };
                }
                const tableData = Object.values(grouped).map((g: any, idx: number) => ({ key: idx, ...g }));
                const selectedProjs = activeGroup.projects.filter((p: any) => compareSelectedProjects.includes(p.project_id));

                // 动态构建列：每个项目一列
                const compareCols = [
                  { title: '大类', dataIndex: 'main_category', width: 80, fixed: 'left' as const, render: (v: string) => <Tag color={getCategoryColor(v)} style={{ margin: 0 }}>{v}</Tag> },
                  { title: '子类', dataIndex: 'sub_category', width: 100, fixed: 'left' as const },
                  { title: '器件名称', dataIndex: 'part_name', width: 140, fixed: 'left' as const, ellipsis: true },
                  { title: '型号', dataIndex: 'part_model', width: 120, fixed: 'left' as const, ellipsis: true },
                  ...selectedProjs.map((proj: any) => ({
                    title: `${proj.project_code}`,
                    dataIndex: 'projects',
                    key: proj.project_id,
                    width: 150,
                    render: (projects: any) => {
                      const p = projects[proj.project_id];
                      if (!p) return <span style={{ color: '#CCC' }}>-</span>;
                      return (
                        <div>
                          <div style={{ fontWeight: 600 }}>¥{p.cost?.toFixed(2) || '0.00'}</div>
                          <div style={{ fontSize: 11, color: '#999' }}>× {p.quantity || 1}</div>
                        </div>
                      );
                    },
                  })),
                ];

                return (
                  <>
                    <Table
                      dataSource={tableData}
                      columns={compareCols}
                      size="small"
                      pagination={false}
                      scroll={{ x: compareCols.reduce((s, c) => s + (c.width || 100), 0) }}
                      rowKey="key"
                    />

                    {/* 可视化对比图 */}
                    <div style={{ marginTop: 20 }}>
                      <b style={{ fontSize: 13 }}>📈 成本对比可视化</b>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginTop: 12 }}>
                        {/* 柱状图：各项目模块成本 */}
                        <Card size="small" title="模块成本对比" style={{ borderRadius: 8 }}>
                          <ReactECharts
                            option={{
                              tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' } },
                              grid: { left: 50, right: 20, top: 20, bottom: 30 },
                              xAxis: { type: 'category', data: selectedProjs.map((p: any) => p.project_code) },
                              yAxis: { type: 'value', name: '成本 (¥)' },
                              series: [{
                                type: 'bar',
                                data: selectedProjs.map((p: any) => ({
                                  value: p.cost,
                                  itemStyle: {
                                    color: getCategoryColor(activeGroup.projects.find((proj: any) => proj.project_id === p.project_id)?.project_type || '其他'),
                                    borderRadius: [6, 6, 0, 0],
                                  },
                                })),
                                label: { show: true, position: 'top', formatter: (params: any) => `¥${params.value.toFixed(2)}`, fontSize: 11 },
                              }],
                            }}
                            style={{ height: 250 }}
                          />
                        </Card>

                        {/* 饼图：各项目器件成本占比 */}
                        <Card size="small" title="器件成本分布对比" style={{ borderRadius: 8 }}>
                          <ReactECharts
                            option={{
                              tooltip: { trigger: 'item', formatter: '{b}: ¥{c} ({d}%)' },
                              legend: { type: 'scroll', bottom: 0, left: 'center', data: selectedProjs.map((p: any) => p.project_code) },
                              series: [{
                                type: 'pie',
                                radius: ['30%', '60%'],
                                center: ['50%', '45%'],
                                data: selectedProjs.map((p: any, idx: number) => ({
                                  name: p.project_code,
                                  value: p.cost,
                                  itemStyle: { color: ['#CF0A2C', '#3B82F6', '#10B981', '#F59E0B', '#8B5CF6'][idx % 5] },
                                })),
                                label: { show: true, formatter: '{b}\n¥{c}', fontSize: 11 },
                              }],
                            }}
                            style={{ height: 250 }}
                          />
                        </Card>
                      </div>
                    </div>
                  </>
                );
              })()}

              {compareSelectedProjects.length < 2 && (
                <div style={{ padding: 20, textAlign: 'center', color: '#999' }}>
                  请至少选择 2 个项目进行对比
                </div>
              )}
            </div>
          )}
        </div>
      )}

      {/* Module create/edit modal */}
      <Modal title={editMod?.id ? '编辑模块' : '新建模块'} open={modModal} onOk={handleSaveMod} onCancel={() => { setModModal(false); setEditMod(null); }} width={460}>
        <Form form={form} layout="vertical">
          <Form.Item label="所属项目" required={!editMod?.id}>
            <Select placeholder="选择项目" value={selProjectForNewMod} onChange={v => setSelProjectForNewMod(v)}
              options={projects.map(p => ({ label: `[${p.code}] ${p.name}`, value: p.id }))} />
          </Form.Item>
          <Form.Item label="模块名称" name="name" rules={[{ required: true }]}><Input placeholder="如: LCM模块" /></Form.Item>
          <Form.Item label="描述" name="description"><Input.TextArea rows={2} /></Form.Item>
        </Form>
      </Modal>

      {/* Item edit modal */}
      <Modal title={editItem?.id ? '编辑器件' : '添加器件'} open={itemModal} onOk={handleSaveItem} onCancel={() => { setItemModal(false); setEditItem(null); }} width={600} destroyOnClose>
        <Form form={form} layout="vertical" initialValues={{ main_category: '硬件类', sub_category: '', quantity: 1, cost: 0 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0 12px' }}>
            <Form.Item label="大类" name="main_category" rules={[{ required: true }]}>
              <Select options={mainCats.map(c => ({ label: c, value: c }))} onChange={(v) => form.setFieldValue('sub_category', (SUB_CATEGORIES[v] || [])[0] || '')} />
            </Form.Item>
            <Form.Item label="子类" name="sub_category"><Select options={(SUB_CATEGORIES[form.getFieldValue('main_category')] || []).map(c => ({ label: c, value: c }))} showSearch /></Form.Item>
            <Form.Item label="数量" name="quantity"><InputNumber min={0} step={0.1} precision={2} style={{ width: '100%' }} /></Form.Item>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 12px' }}>
            <Form.Item label="器件名称" name="part_name" rules={[{ required: true }]}><Input /></Form.Item>
            <Form.Item label="型号" name="part_model"><Input /></Form.Item>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 12px' }}>
            <Form.Item label="单价" name="cost"><InputNumber min={0} precision={2} style={{ width: '100%' }} prefix="¥" /></Form.Item>
            <Form.Item label="备注" name="remark"><Input /></Form.Item>
          </div>
        </Form>
      </Modal>
    </div>
  );
}
